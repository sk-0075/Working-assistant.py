...
def run_alexa():
    ...
    if 'play' in command:
        ...
        time = datetime.datetime.now().strftime()time('%I:%M %p')
        print(time)
        talk('current shiv shambhu time is' + time)
    elif 'who the hell is' in command:
        person = command.replace('who thw hell is', '')
        ...
        talk('8544754745')
    elif'kaun hai tera malik' in command:
